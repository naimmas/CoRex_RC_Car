 ...
 */
static response_status_t write_register(qmi86_dev_t* ppt_dev, uint8_t* ppt_data,
...
static response_status_t wait_for_statusint_bit(qmi86_dev_t* ppt_dev,
                                                uint8_t      p_desired_bit_val,
                                                uint8_t      p_bit_no)
{
    ...
    while (ret_val == RET_OK && bit_ok == FALSE)
    {
        ...
        bit_ok =
          (((reg_data & (1 << p_bit_no)) >> p_bit_no) == p_desired_bit_val);
...
static bool_t is_dev_ready(qmi86_dev_t* ppt_dev)
{
    ...
    return ((ret_val == RET_OK)
            && ((chip_id == QMI86_CHIP_ID) && (who_am_i == QMI86_WHO_AM_I)));
}
...
static response_status_t read_firmware_version(qmi86_dev_t* ppt_dev)
{
    ...
    ret_val  = read_register(ppt_dev,
                            &ppt_dev->chip_fw_version.u8_arr[0],
                            1,
                            QMI86_REG_DQW_L);
    ret_val |= read_register(ppt_dev,
                             &ppt_dev->chip_fw_version.u8_arr[1],
                             1,
                             QMI86_REG_DQW_H);
    ret_val |= read_register(ppt_dev,
                             &ppt_dev->chip_fw_version.u8_arr[2],
                             1,
                             QMI86_REG_DQX_L);
...
static response_status_t read_chip_id(qmi86_dev_t* ppt_dev)
{
    ...
    ret_val =
      read_register(ppt_dev, &ppt_dev->chip_id.u8_arr[0], 1, QMI86_REG_DVX_L);
    ret_val |=
      read_register(ppt_dev, &ppt_dev->chip_id.u8_arr[1], 1, QMI86_REG_DVX_H);
    ret_val |=
      read_register(ppt_dev, &ppt_dev->chip_id.u8_arr[2], 1, QMI86_REG_DVY_L);
    ret_val |=
      read_register(ppt_dev, &ppt_dev->chip_id.u8_arr[3], 1, QMI86_REG_DVY_H);
...
static qmi86_st_result self_test_accel(qmi86_dev_t* ppt_dev)
{
    ...
    {
        reg_data =
          QMI_OWR_BITS(reg_data, QMI86_REG_CTRL2_AODR, QMI86_ACC_ODR_1000_HZ);
...
response_status_t dd_qmi86_get_data_settings(qmi86_dev_t* ppt_dev)
{
    ...
    if (ret_val == RET_OK)
    {
        ...
        hndlr->acc_lpf_en =
          QMI_GET_BITS(reg_content[2], QMI86_REG_CTRL5_ALPF_EN);
        hndlr->acc_lpf_mode =
          QMI_GET_BITS(reg_content[2], QMI86_REG_CTRL5_ALPF_MODE);

        hndlr->gyro_lpf_en =
          QMI_GET_BITS(reg_content[2], QMI86_REG_CTRL5_GLPF_EN);
        hndlr->gyro_lpf_mode =
          QMI_GET_BITS(reg_content[2], QMI86_REG_CTRL5_GLPF_MODE);
    }
    else
    {
        // Nothing to do
...
response_status_t dd_qmi86_snooze_gyro(qmi86_dev_t* ppt_dev, bool_t p_snooze)
{
    ...
    {
        ctrl7_reg_data =
          QMI_OWR_BITS(ctrl7_reg_data, QMI86_REG_CTRL7_GSN, p_snooze);
                                           ...
                                           qmi86_sensor_mode p_dev_mode)
{
    ...
    if (ret_val == RET_OK)
    {
        ctrl1_reg_data = QMI_OWR_BITS(ctrl1_reg_data,
...
{
    ASSERT_AND_RETURN(ppt_dev == NULL, QMI86_ST_RESULT_API_ERROR);
    ASSERT_AND_RETURN(p_sensor_type >= QMI86_SENSORS_CNT,
...
response_status_t dd_qmi86_init(qmi86_dev_t** ppt_dev, qmi86_dev_id_t p_dev_id)
{
    ...
    if (ret_val == RET_OK)
    {
        ...
        if (is_dev_ready(pt_curr_dev) == TRUE
