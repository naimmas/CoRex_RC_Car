...
response_status_t serial_ifc_init(void) {
    ...
    if (ret == RET_OK)
    {
        ...
        ret = su_rb_init(&log_buffer, log_buffer_data, sizeof(log_buffer_data)) ? RET_OK : RET_ERROR;
        ret |= su_rb_is_ready(&log_buffer) ? RET_OK : RET_ERROR; // Check if the buffer is ready
    }
    if (ret == RET_OK)
    {
        ha_uart_dma_register_callback(UART_PORT1, dma_cb); // Register DMA callback
    }
    return ret;
}
